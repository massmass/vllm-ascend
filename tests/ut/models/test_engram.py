...
def test_fused_gate_falls_back_for_cpu_inputs(monkeypatch):
    ...
    # Patch the environment variable itself. ``envs`` serves this name through
    # module ``__getattr__``, so monkeypatch.setattr would leave a real module
    # attribute behind after undo and shadow the environment for later tests.
